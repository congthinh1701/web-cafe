...
function FormAddress() {
  ...
  // const valueCity=selectedCity.label
  // const valueDistrict=selectedDistrict.label
  // const a=Object.assign(selectedCity.label,selectedDistrict.label,selectedWard.label)
  
  const valueObject =Object.assign({...value, selectedCity,selectedDistrict,selectedWard})

  const { emailError, nameError, phoneError, addressError } = stateValidate;

  const submit = (values) => {
    values.preventDefault();
    console.log( valueObject.selectedCity.label);
  };

  // transmit data go up redux

  const actionValue = valueForm({value,stateValidate});
