...
function Payment() {
  ...
  const {emailValue,nameValue,phoneValue,addressValue}= valueForm.valueObject
  const { emailError,nameError,phoneError,addressError }=valueForm.stateValidate
  const submit = () => {
    console.log(valueForm.stateValidate);
    if ( emailValue && nameValue && phoneValue && addressValue && !emailError && !nameError && !phoneError && !addressError) {
      console.log('dsads');
    }else{
